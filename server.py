...
MAX_WORKERS = int(os.getenv("MARKET_DATA_WORKERS", "4"))
USER_AGENT = (
...
def fetch_official_constituents_csv() -> str:
    """Fetch the official NIFTY 500 CSV, with repository CSV fallback."""
    try:
        raw = fetch_bytes(
            CONSTITUENTS_URL,
            timeout=min(HTTP_TIMEOUT_SECONDS, 25),
            attempts=1,
        )
        text = raw.decode("utf-8-sig")
        # Validate before returning so a malformed official response does not
        # poison the scanner.
        parse_constituents(text)
        return text
    except Exception as exc:
        local_path = Path(__file__).resolve().parent / "nifty500.csv"
        if not local_path.exists():
            raise UpstreamDataError(
                f"Official NIFTY 500 constituent download failed and local "
                f"nifty500.csv was not found: {exc}"
            ) from exc
        try:
            ...
            raise UpstreamDataError(
                ...
                f"local nifty500.csv could not be parsed: {local_exc}"
            ) from local_exc
...
TIMEFRAME_CONFIG = {
    "YIT": {"location": "1y", "trend": "6mo", "execution": "3mo", "interval": "3mo"},
    "HYIT": {"location": "6mo", "trend": "3mo", "execution": "1mo", "interval": "1mo"},
    "QIT": {"location": "3mo", "trend": "1mo", "execution": "1wk", "interval": "1wk"},
    "MIT": {"location": "1mo", "trend": "1wk", "execution": "1d", "interval": "1d"},
    "WIT": {"location": "1wk", "trend": "1d", "execution": "125m", "interval": "5m"},
}

def aggregate_125m(candles: list[dict[str, Any]]) -> list[dict[str, Any]]:
    grouped=[]; by_date={}
    for c in candles: by_date.setdefault(c["date"], []).append(c)
    for date, day in sorted(by_date.items()):
        day=sorted(day,key=lambda c:c.get("timestamp") or "")
        for start in range(0,len(day),25):
            chunk=day[start:start+25]
            if len(chunk)<2: continue
            opens=[c.get("open") for c in chunk if c.get("open") is not None]
            highs=[c.get("high") for c in chunk if c.get("high") is not None]
            lows=[c.get("low") for c in chunk if c.get("low") is not None]
            closes=[c.get("close") for c in chunk if c.get("close") is not None]
            vols=[c.get("volume") for c in chunk if c.get("volume") is not None]
            if not (opens and highs and lows and closes): continue
            grouped.append({"date":date,"timestamp":chunk[-1].get("timestamp"),"open":opens[0],"high":max(highs),"low":min(lows),"close":closes[-1],"adjustedClose":closes[-1],"volume":int(sum(vols)) if vols else None})
    return grouped

def fetch_history(symbol: str, lookback: int, interval: str = "1d") -> list[dict[str, Any]]:
    ...
    if interval == "5m": calendar_days = 59
    elif interval == "1wk": calendar_days = max(730, int(lookback * 9))
    elif interval == "1mo": calendar_days = max(3650, int(lookback * 35))
    elif interval == "3mo": calendar_days = max(7300, int(lookback * 100))
    else: calendar_days = max(365, int(lookback * 2.5))
    ...
    url = (
        ...
        f"&interval={quote(interval, safe='')}&events=history&includeAdjustedClose=true"
    )
    payload = json.loads(fetch_bytes(url, attempts=4).decode("utf-8"))
    ...
    if interval == "5m": candles = aggregate_125m(candles)
...
def detect_zones(candles: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    # Count structural breaks after each zone forms. A break is counted only
    # when price crosses the zone boundary from the inside to the outside.
    # This keeps the user's intended sequence intact: first break = Sideways,
    # second break = Trend. The six evaluator rules above are not changed.
    for zone in candidates:
        try:
            base_index = next(
                i for i, candle in enumerate(candles)
                if candle.get("date") == zone.get("baseDate")
            )
        ...
        previous_value = None
        if zone["type"] == "supply":
            boundary = zone.get("high")
            for candle in candles[base_index + 1 :]:
                value = candle.get("high")
                if value is None or boundary is None:
                    continue
                if previous_value is not None and previous_value <= boundary < value:
                    breaks += 1
                previous_value = value
            zone["breakCount"] = breaks
            zone["trend"] = "UP Trend" if breaks >= 2 else ("Sideways" if breaks == 1 else "—")
            zone["achievement"] = "UP Achievement" if breaks >= 2 else ("Sideways" if breaks == 1 else "—")
        else:
            boundary = zone.get("low")
            for candle in candles[base_index + 1 :]:
                value = candle.get("low")
                if value is None or boundary is None:
                    continue
                if previous_value is not None and previous_value >= boundary > value:
                    breaks += 1
                previous_value = value
            zone["breakCount"] = breaks
            zone["trend"] = "DOWN Trend" if breaks >= 2 else ("Sideways" if breaks == 1 else "—")
            zone["achievement"] = "DOWN Achievement" if breaks >= 2 else ("Sideways" if breaks == 1 else "—")
    ...
    for record in records:
        ...
        for zone in record.get("zones") or []:
            ...
            if zone_type not in ("supply", "demand"):
                continue
            # Recommendations are UP Trend Supply only: two or more supply
            # boundary breaks are required. Demand zones stay available in the
            # DZ view but are not promoted to the recommendation list.
            if zone_type == "supply" and zone.get("breakCount", 0) < 2:
                continue
            ...
            candidates.append(
                {
                    ...
                    "zone": {
                        ...
                        "breakCount": zone.get("breakCount", 0),
                        "trend": zone.get("trend", "—"),
                        "achievement": zone.get("achievement", "—"),
                    },
...
class MarketDataService:
    ...
    def get_market_data(self, universe: str, lookback: int, timeframe: str = "YIT") -> dict[str, Any]:
        timeframe=timeframe.upper()
        if timeframe not in TIMEFRAME_CONFIG: raise ValueError(f"Unsupported timeframe: {timeframe}")
        interval=TIMEFRAME_CONFIG[timeframe]["interval"]
        cache_key=(universe, lookback, timeframe)
        now = time.monotonic()
        with self._lock:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
            futures = {
                executor.submit(fetch_history, item["symbol"], lookback, interval): item
        ...
        # Controlled retry pass for transient Yahoo failures.
        if failures:
            failed_symbols = {item["symbol"] for item in failures}
            retry_failures = []
            for constituent in constituents:
                if constituent["symbol"] not in failed_symbols:
                    continue
                try:
                    candles = fetch_history(constituent["symbol"], lookback, interval)
                    records.append(build_symbol_record(constituent, candles))
                except (UpstreamDataError, ValueError, json.JSONDecodeError) as exc:
                    retry_failures.append({"symbol": constituent["symbol"], "error": str(exc)})
                time.sleep(0.08)
        ...
        response = {
            ...
            "meta": {
                ...
                "lookback": lookback,
                "timeframe": timeframe,
                "timeframeMapping": TIMEFRAME_CONFIG[timeframe],
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def handle_market_data(self, query_string: str) -> None:
        ...
        if universe != "NIFTY500":
            ...
            return
        timeframe = query.get("timeframe", ["YIT"])[0].upper()
        ...
        if timeframe not in TIMEFRAME_CONFIG:
            self.send_json(HTTPStatus.BAD_REQUEST, {"ok": False, "error": {"code": "invalid_timeframe", "message": "timeframe must be YIT, HYIT, QIT, MIT or WIT"}})
            return
        try:
            response = market_data.get_market_data(universe, lookback, timeframe)
