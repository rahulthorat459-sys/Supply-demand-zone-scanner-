...
def fetch_bytes(
    ...
    """Fetch real upstream bytes; retry with curl on hosted-runtime failures.

    The retry uses the exact same upstream URL and does not introduce
    synthetic or placeholder market data.
    """
    ...
    for attempt in range(attempts):
        ...
        try:
            ...
            last_error = UpstreamDataError(
                f"curl failed ({result.returncode})" + (f": {detail}" if detail else "")
            )
