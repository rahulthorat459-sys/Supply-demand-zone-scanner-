...
BUNDLED_CONSTITUENTS_FILE = Path(
    os.getenv("NIFTY500_BUNDLED_FILE", str(BASE_DIR / "nifty500.csv"))
)
CONSTITUENTS_CACHE_FILE = Path(
    os.getenv("NIFTY500_CONSTITUENTS_CACHE_FILE", str(BASE_DIR / "nifty500_cache.csv"))
)
STATIC_HTML_FILE = BASE_DIR / "index.html"
HTTP_TIMEOUT_SECONDS = float(os.getenv("MARKET_DATA_TIMEOUT_SECONDS", "45"))
CONSTITUENTS_TIMEOUT_SECONDS = float(
    os.getenv("NIFTY500_CONSTITUENTS_TIMEOUT_SECONDS", "15")
)
MAX_WORKERS = int(os.getenv("MARKET_DATA_WORKERS", "6"))
USER_AGENT = (
...
def load_constituents() -> list[dict[str, str]]:
    ...
    try:
        ...
        source = "live"
    except UpstreamDataError as official_error:
        # Render/cloud runtimes can intermittently time out when reaching the
        # official NSE Indices CSV. The repository contains a validated NIFTY
        # 500 constituent snapshot specifically so the scanner can still use
        # real Yahoo Finance market data without inventing symbols or prices.
        if stale_disk_cache is not None:
            constituents = stale_disk_cache
            source = "stale_disk_cache"
        elif BUNDLED_CONSTITUENTS_FILE.exists():
            try:
                bundled = parse_constituents(
                    BUNDLED_CONSTITUENTS_FILE.read_text(encoding="utf-8-sig")
                )
            except (OSError, UnicodeError, UpstreamDataError):
                bundled = None
            if bundled is None:
                raise official_error
            constituents = bundled
            source = "bundled_repo"
        else:
            raise official_error
        with _constituents_cache_lock:
            _constituents_cache = (now, list(constituents))
            _constituents_cache_source = source
        return constituents
    try:
...
def fetch_history(symbol: str, lookback: int) -> list[dict[str, Any]]:
    ...
    hosts = [YAHOO_CHART_URL.rstrip("/")]
    if "query2.finance.yahoo.com" not in hosts:
        hosts.append("https://query2.finance.yahoo.com/v8/finance/chart")
    last_error: Exception | None = None
    result = None
    for host in hosts:
        url = (
            f"{host}/{quote(yahoo_symbol, safe='')}"
        ...
        except (UpstreamDataError, json.JSONDecodeError, UnicodeDecodeError) as exc:
            last_error = exc
    if result is None:
        raise UpstreamDataError(str(last_error or f"Yahoo Finance returned no history for {yahoo_symbol}"))
...
class ApiHandler(BaseHTTPRequestHandler):
    ...
    def do_HEAD(self) -> None:
        """Handle Render/browser HEAD probes without returning 501."""
