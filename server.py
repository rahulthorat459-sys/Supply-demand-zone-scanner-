...
def fetch_official_constituents_csv() -> list[dict[str, str]]:
    """Use official NIFTY 500 constituents, with repository CSV fallback."""
    try:
        raw = fetch_bytes(NIFTY500_CSV_URL, timeout=min(HTTP_TIMEOUT_SECONDS, 25), attempts=1)
        rows = parse_nifty500_csv(raw)
        if rows:
            return rows
        raise UpstreamDataError("Official NIFTY 500 CSV returned no constituents")
        ...
        try:
            rows = parse_nifty500_csv(local_path.read_bytes())
        ...
        if not rows:
            raise UpstreamDataError(
                f"Official NIFTY 500 constituent download failed: {exc}; "
                "local nifty500.csv contains no constituents"
            )
        return rows
