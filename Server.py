...
class MarketDataService:
    ...
    def get_market_data(self, universe: str, lookback: int) -> dict[str, Any]:
        ...
        # Keep full candles only for server-side evaluation. Do not retain
        # them in the API response/cache: 500 symbols × 500 candles causes
        # unnecessary Render memory pressure.
        public_records = [
            {key: value for key, value in record.items() if key != "candles"}
            for record in records
        ]
        records.clear()
